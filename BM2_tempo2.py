...
# Smar remplacé par olat et Salter par Gang, script valable pour 2sp
...
# Écriture entête CSV
        header = [
            "OG", "Bloc", "Olatifolia", "Gangustifolia", "Sum",
...
# Création ensemble (set) vide pour lister les types d'espèces sous ce nœud sans doublons
                    ...
                    for l in node.get_leaves():
                        ...
                        lname = l.name.lower()
                        # Si mot "Olatifolia" détecté @nom de la feuille
                        if "olatifolia" in lname:
                            # alors ajout tag "olat" @ensemble d'espèces
                            species_found.add("olat")
                        # Si "Gangustifolia" détecté @nom de la feuille
                        elif "gangustifolia" in lname:
                            # alors ajout tag "gang" @ensemble d'espèces
                            species_found.add("gang")
...
# CONDITION : Filtre strictement les deux espèces ET aucun intrus (Le plus gros bloc pur)
                    if "olat" in species_found and "gang" in species_found and "intrus" not in species_found:
...
# VERROU : (Pour n'avoir que Bloc 1 et Bloc 4 ; évite les sous blocs redondants)
                        ...
                        # Comptage nb sequences Olatifolia @bloc
                        olat_count = len([l for l in node.get_leaves() if "olatifolia" in l.name.lower()])
                        # Comptage nb sequences Gangustifolia @bloc
                        gang_count = len([l for l in node.get_leaves() if "gangustifolia" in l.name.lower()])
                        # Calcul somme totale sequences cibles réunis @bloc (colonne "Sum") à verfier changement "gene_sum"
                        seq_sum = olat_count + gang_count
##################################################################################################
...
###########BOUCLE
                        ...
                        for i in range(4):
                            if i < len(all_S_sorted):
                                ...
                                SD_sp1_list.append(get_first_d_dist(current_spec, "olatifolia"))
                                SD_sp2_list.append(get_first_d_dist(current_spec, "gangustifolia"))
                                    
                                # Grille de classification stricte SI NOUVELLES ESPECE, SCENARIO ET CLASSIFICAITON A MODIFIER ICI 
                                if nb_dup_pre >= 1:
                                    s_classifications.append("DS")
                                elif nb_dup_pre == 0 and nb_dup_post >= 1:
                                    s_classifications.append("SD")
                                elif nb_dup_pre == 0 and nb_dup_post == 0:
...
##################################################################################################
                        row = [
                            f'"{og_id}"', f'"{bloc_id}"', f'"{olat_count}"', f'"{gang_count}"', f'"{seq_sum}"',
