...
#Smar remplacé par Zmays et Salter par Sbico
...
# Création ensemble (set) vide pour lister les types d'espèces sous ce nœud sans doublons
                    ...
                    for l in node.get_leaves():
                        ...
                        lname = l.name.lower()
                        # Si mot "Zmays" détecté @nom de la feuille
                        if "zmays" in lname:
                            # alors ajout tag "Zmays" @ensemble d'espèces
                            species_found.add("zmays")
                        # Si "Sbicolor" détecté @nom de la feuille
                        elif "sbicolor" in lname:
                            # alors ajout tag "sbico" @ensemble d'espèces
                            species_found.add("sbico")
                        # Si autre plante (Acomosus, smaritimus, etc) => intrus phylogénétique
                        else:
...
# CONDITION : Filtre strictement les deux espèces ET aucun intrus (Le plus gros bloc pur)
                    if "zmays" in species_found and "sbico" in species_found and "intrus" not in species_found:
...
# Ajout down14+19 => blocs avec 0 spéciation sont ignorés et + de 4 spéciations (selon scénario établi)
...
# VERROU : (Pour n'avoir que Bloc 1 et Bloc 4 ; évite les sous blocs redondants)
                        ...
                        # len([...]) => Compte nb de sequences individuelles de chaque espèce dans ce bloc isolé
                        # Comptage nb sequences Zmays @bloc
                        zmays_count = len([l for l in node.get_leaves() if "zmays" in l.name.lower()])
                        # Comptage nb sequences Sbicolor @bloc
                        sbico_count = len([l for l in node.get_leaves() if "sbico" in l.name.lower()])
                        # Calcul somme totale sequences cibles réunis @bloc (colonne "Sum") à verfier changement "gene_sum"
                        seq_sum = zmays_count + sbico_count
##################################################################################################
...
##################################################################################################
                        ...
                        # Extraction des distances pour S1 (garantie par le filtre) et S2 (conditionnelle)
                        ...
                        #AJOUT down21
                        def get_first_d_dist(spec_node, sp_name):
                            target_child = None
                            for child in spec_node.children:
                                if any(sp_name in l.name.lower() for l in child.get_leaves()):
                                    target_child = child
                                    break
                            if not target_child:
                                return "N/A"
                            ...
                            queue_d = [target_child]
                            while queue_d:
                                c = queue_d.pop(0)
                                if c.name and c.name.upper().startswith("D"):
                                    return f"{c.get_distance(spec_node):.6f}"
                                if not c.is_leaf():
                                    for grandchild in c.children:
                                        if any(sp_name in l.name.lower() for l in grandchild.get_leaves()):
                                            queue_d.append(grandchild)
                            return "N/A" 
...
###########BOUCLE
                        ...
                        for i in range(4):
                            if i < len(all_S_sorted):
                                ...
                                SD_sp1_list.append(get_first_d_dist(current_spec, "zmays"))
                                SD_sp2_list.append(get_first_d_dist(current_spec, "sbicolor"))
                                #test debug tempo1 11/06    
                                print(f"DEBUG S{i+1}: nb_dup_pre={nb_dup_pre} (type={type(nb_dup_pre)}), nb_dup_post={nb_dup_post} (type={type(nb_dup_post)})")

                                # SI NOUVELLES ESPECE, SCENARIO ET CLASSIFICAITON A MODIFIER ICI 
                                if nb_dup_pre >= 1:
...
################################################################################ #############
                        ...
                        has_absD = "absD" in s_classifications #ajout 5/06 down21.2
                        ...
                        t_bloc_absD = s_classifications.count("absD")
...
##################################################################################################
                        row = [
                            f'"{og_id}"', f'"{bloc_id}"', f'"{zmays_count}"', f'"{sbico_count}"', f'"{seq_sum}"',
                            f'"{total_dup_bloc}"', f'"{total_spec_bloc}"', f'"{total_t_bloc}"', f'"{root_type}"', 
                            f'"{dist_1st_spec:.6f}"', f'"{dist_2nd_spec:.6f}"',
                            ...
                            f'"{type_scenario}"',
                            f'"{t_bloc_DS}"', f'"{t_bloc_SD}"', f'"{t_bloc_absD}"', f'"{t_bloc_autres}"', # Les 3 nouvelles colonnes
                            f'"{bloc_newick}"'
                        ]
...
###########################################################################################
    with open(output_csv, "a") as f_out_append:
        ...
        f_out_append.write(f"# Catégorie DS  : {summary_counts['DS']} bloc(s)\n")
